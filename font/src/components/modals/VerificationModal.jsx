...
const VerificationModal = ({ onClose, onVerify, user }) => {
    ...
    const handleRequestVerification = async () => {
        setIsLoading(true);
        try {
            const response = await fetch('http://localhost:8000/api/request-verification/', {
                method: 'POST',
                headers: {
            ...
            if (response.ok) {
                ...
                console.error('Failed to request verification');
                setIsLoading(false);
            }
        } catch (error) {
    ...
    const handleVerifyOTP = async (e) => {
        ...
        try {
            const response = await fetch('http://localhost:8000/api/verify-otp/', {
                ...
                body: JSON.stringify({ user: user.id, otp })
            });
